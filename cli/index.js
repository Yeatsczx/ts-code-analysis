...
const { REPORTDEFAULTDIR } = require(path.join(__dirname, '../lib/constant'));    // 常量模块
...
program
    ...
    .action(async () => {
        try{
            ...
            // 是否存在analysis.config.js配置文件
            if(isConfig){
                let config =require(configPath);
                // 配置文件中必填配置项scanSource是否为空
                if(config.scanSource && Array.isArray(config.scanSource) && config.scanSource.length>0){
                    ...
                    // scanSource参数选项必填属性是否为空
                    if(!isParamsError){
                        // 配置文件中待分析文件目录是否存在
                        if(!isCodePathError){
                            // 配置文件中是否存在必填配置项analysisTarget
